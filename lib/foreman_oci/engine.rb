module ForemanPluginTemplate
  class Engine < ::Rails::Engine
    ...
    initializer 'foreman_oci.load_app_instance_data' do |app|
      ForemanPluginTemplate::Engine.paths['db/migrate'].existent.each do |path|
        app.config.paths['db/migrate'] << path
      end
    end

    initializer 'foreman_oci.register_plugin', :before => :finisher_hook do |_app|
      Foreman::Plugin.register :foreman_oci do
        ...
        # Add permissions
        security_block :foreman_oci do
          permission :view_foreman_oci, { :'foreman_oci/example' => [:new_action],
                                                      :'react' => [:index] }
        end

        # Add a new role called 'Discovery' if it doesn't exist
        role 'ForemanPluginTemplate', [:view_foreman_oci]
        ...
        widget 'foreman_oci_widget', name: N_('Foreman plugin template widget'), sizex: 4, sizey: 1
      end
    end
